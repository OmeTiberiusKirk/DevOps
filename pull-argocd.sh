#!/bin/bash

RUNNER_IP="harbor.local"

echo "Login Docker ที่ $RUNNER_IP"
docker login https://$RUNNER_IP -u admin

echo "ตรวจสอบ Docker Images"
# Argocd
VERSION="v3.4.5"
IMAGE="quay.io/argoproj/argocd:$VERSION"
LOCAL_IMAGE="$RUNNER_IP/argocd/argocd:$VERSION"
echo $IMAGE
if [ -n "$(docker images -q "$LOCAL_IMAGE" 2>/dev/null)" ]; then
	echo "✅ [SUCCESS] พบ Docker Image: $LOCAL_IMAGE อยู่บนระบบเรียบร้อยแล้ว!"
else
	echo "❌ [WARNING] ไม่พบ Docker Image: $LOCAL_IMAGE ในเครื่องนี้"
	docker pull $IMAGE
	docker tag $IMAGE $LOCAL_IMAGE
	docker push $LOCAL_IMAGE
fi

# Dex
VERSION="v2.45.0"
IMAGE="ghcr.io/dexidp/dex:$VERSION"
LOCAL_IMAGE="$RUNNER_IP/argocd/dex:$VERSION"
if [ -n "$(docker images -q "$LOCAL_IMAGE" 2>/dev/null)" ]; then
	echo "✅ [SUCCESS] พบ Docker Image: $LOCAL_IMAGE อยู่บนระบบเรียบร้อยแล้ว!"
else
	echo "❌ [WARNING] ไม่พบ Docker Image: $LOCAL_IMAGE ในเครื่องนี้"
	docker pull $IMAGE
	docker tag $IMAGE $LOCAL_IMAGE
	docker push $LOCAL_IMAGE
fi


# Redis
VERSION="8.2.3-alpine"
IMAGE="public.ecr.aws/docker/library/redis:$VERSION"
LOCAL_IMAGE="$RUNNER_IP/argocd/redis:$VERSION"
if [ -n "$(docker images -q "$LOCAL_IMAGE" 2>/dev/null)" ]; then
	echo "✅ [SUCCESS] พบ Docker Image: $LOCAL_IMAGE อยู่บนระบบเรียบร้อยแล้ว!"
else
	echo "❌ [WARNING] ไม่พบ Docker Image: $LOCAL_IMAGE ในเครื่องนี้"
	docker pull $IMAGE
	docker tag $IMAGE $LOCAL_IMAGE
	docker push $LOCAL_IMAGE
fi